...
@kt.case(atol=1e-3, rtol=1e-3, ntest=4)
def test_flash_attention_fw():
    batch_size, seq_len = kt.bs_sl()
    nhead = kt.nhead
    head_dim = int(np.random.choice([16, 32, 64]))
    ...
    def custom():
        ...
        out = torch.tensor(out_mt.to_numpy(), dtype=torch.float32, device=kt.device)
...
kt.run("test_flash_attention_fw")
