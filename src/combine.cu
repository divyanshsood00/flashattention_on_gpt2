    ...
    int fn_id)
{
  ...
  // Broadcast the out_index to the a_index according to a_shape
  broadcast_index(out_index, out_shape, a_shape, a_index, out_shape_size, a_shape_size);
  
  // Calculate the position of element in a_array according to a_index and a_strides
  int a_pos = index_to_position(a_index, a_strides, a_shape_size);
  
  // Broadcast the out_index to the b_index according to b_shape
  broadcast_index(out_index, out_shape, b_shape, b_index, out_shape_size, b_shape_size);
  
  // Calculate the position of element in b_array according to b_index and b_strides
  int b_pos = index_to_position(b_index, b_strides, b_shape_size);
  
  // Apply the binary function to the input elements in a_array & b_array and write the output to the out memory
    ...
    int fn_id)
{
  ...
  /// BEGIN HW1_3
  // Define the position of the output element that this thread will write to
  ...
  // Initialize the accumulator with the reduce_value
  ...
  int final_out_pos = index_to_position(out_index, out_strides, shape_size);
  out[final_out_pos] = acc;
  /// END HW1_3
}
    ...
    const int *b_strides)
{
  ...
  /// BEGIN HW1_4
  // Compute the row and column of the output matrix this thread will compute
  ...
  // Iterate over tiles of the two input matrices
  ...
  for (int tile = 0; tile < num_tiles; tile++) {
    ...
    // Compute the output tile for this thread
    for (int k = 0; k < TILE; k++) {
      acc += a_shared[threadIdx.y][k] * b_shared[k][threadIdx.x];
    }
    
    // Synchronize to make sure all threads are done computing before loading next tile
    __syncthreads();
  }
